...
class Offtarget:
    ...
    def __newFwd(self, chromosome, strand, start, substitutions, sequence, lengthSeq, lengthPAM, coreRange):
        ...
        # check for cases it is a str
        if isinstance(coreRange, str):   
            if coreRange == 'NA':
                self.alignment = "PAM" + "".join(self.alignment)
            else:
                print('Error, this case should not occur')
        # check for cases it is not a string (assume it is an integer)
        else: 
            if coreRange > 0:
                self.alignment = "PAM[" + "".join(self.alignment[:coreRange]) + "]" + "".join(self.alignment[coreRange:])
            else:
...
class sgRNAbindingSite:    
    def __init__(self, sequence, position, strand, fwdPrimer, revPrimer, pamLength):
        ...
        if fwdPrimer == "TAGG":  # T7
            if self.sequence[0] == 'G' and self.sequence[1] == 'G':
                self.oligo1 = 'TA' + self.sequence[0:-pamLength]
                self.oligo2 = 'AAAC' + reverse_complement(self.sequence[2:-pamLength])
            elif self.sequence[0] == 'G' and not self.sequence[1] == 'G':
                self.oligoAfwd = 'TAg' + self.sequence[0:-pamLength]
                self.oligoArev = 'AAAC' + reverse_complement(self.oligoAfwd[4:])
                self.oligoSfwd = 'TAGg' + self.sequence[2:-pamLength]
                self.oligoSrev = 'AAAC' + reverse_complement(self.oligoSfwd[4:])
            else:
                self.oligoAfwd = 'TAgg' + self.sequence[0:-pamLength]
                self.oligoArev = 'AAAC' + reverse_complement(self.oligoAfwd[4:])
                self.oligoSfwd = 'TAgg' + self.sequence[2:-pamLength]
                self.oligoSrev = 'AAAC' + reverse_complement(self.oligoSfwd[4:])
        elif fwdPrimer == "CACCG":  # U6
            if self.sequence[0] == 'G':
                self.oligo1 = 'CACC' + self.sequence[0:-pamLength]
                self.oligo2 = 'AAAC' + reverse_complement(self.sequence[:-pamLength])
            else:
                self.oligoAfwd = 'CACCg' + self.sequence[0:-pamLength]
                self.oligoArev = 'AAAC' + reverse_complement('G' + self.oligoAfwd[5:])
                self.oligoSfwd = 'CACCg' + self.sequence[1:-pamLength]
                self.oligoSrev = 'AAAC' + reverse_complement('G' + self.oligoSfwd[5:])
        else:
            self.oligo1 = fwdPrimer + self.sequence[0:-pamLength]
            self.oligo2 = revPrimer + reverse_complement(self.sequence[0:-pamLength])

    def addOffTargets(self, offTargets, coordinates):
...
class sgRNAbindingSites:
    def __init__(self):
        self.sites = []
    def add(self, sequence, position, strand, fwdPrimer, revPrimer, pamLength):
        newSite = sgRNAbindingSite(sequence, position, strand, fwdPrimer, revPrimer, pamLength)
...
def doSearch(name, query, pamType, targetSize, totalMismatches, coreLength, coreMismatches, sgRNA5, sgRNA3, fwdPrimer, revPrimer, outputFolder, bowtiePath, indexPath, exonsFile, genesFile, maxOT):
    ...
    if(len(candidates.sites) < 1):
        ...
        print('No candidates found in the query sequence named %s.' % name)
        return
    ...
    for idx in range(len(candidates.sites)):       
        ...
        if maxOT < float("inf"):
            candidates.sites[idx].addOffTargets(ot[:maxOT], coordinates)
        else:
            candidates.sites[idx].addOffTargets(ot, coordinates)
        ...
        print("Sequence %s: Done with candidate %i out of %i.\n" % (name, idx + 1, len(candidates.sites)))
...
if __name__ == "__main__":
    ...
    if seqLine != "":
        ...
        sys.stderr.write("It looks that your file is not (multi)fasta format. Please check it and try again.")
        sys.exit(1)
